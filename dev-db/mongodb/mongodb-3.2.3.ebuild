...
inherit eutils flag-o-matic multilib pax-utils scons-utils systemd toolchain-funcs user versionator check-reqs python-r1
...
SRC_URI="https://fastdl.mongodb.org/src/${MY_P}.tar.gz
		arm? ( https://ftp.mozilla.org/pub/mozilla.org/firefox/releases/38.4.0esr/source/firefox-38.4.0esr.source.tar.bz2 )"
...
src_unpack() {
	unpack ${A}
	use arm && mv ${WORKDIR}/mozilla-esr38 ${S}/src/third_party/mozjs-38/mozilla-release
}
...
pkg_setup() {
	...
	(use x86 || use arm) && scons_opts+=( --wiredtiger=off )
...
src_prepare() {
	...
	epatch_user
	if use arm; then
		echo 'env.Append( CCFLAGS=["-std=gnu++11"] )' >> src/third_party/gperftools-2.2/SConscript
		cd src/third_party/mozjs-38
		python_setup 'python2*'
		./gen-config.sh arm linux || die
	fi
}
